...
import { IKhoCoty, nameBrandFilters } from "../../../../types/IKhoCoty";
...
const KhoCotyTable: FC<RecentOrdersTableProps> = ({
  dataTables,
  updateDataTable,
  handleClickOpen,
}) => {
  ...
  const filteredCustomerOrders = dataTables.filter(
    (order) =>
      order.kid.toLowerCase().includes(searchQuery.toLowerCase()) ||
      order.nameK.toLowerCase().includes(searchQuery.toLowerCase()) ||
      order.numberBox.toLowerCase().includes(searchQuery.toLowerCase()) ||
      order.wtid.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const sortedCustomerOrders = [...filteredCustomerOrders].sort((a, b) => {
    if (sortOrder === "asc") {
      return a[sortBy as keyof IKhoCoty]! < b[sortBy as keyof IKhoCoty]!
        ? -1
        : 1;
    } else {
      return a[sortBy as keyof IKhoCoty]! > b[sortBy as keyof IKhoCoty]!
      ...
      {selectedBulkActions && (
        <Box flex={1} p={2}>
          <BulkActions
            ...
            handleShipSelectedCustomerOrders={handleShipSelectedCustomerOrders}
          />
        </Box>
      )}
      {!selectedBulkActions && (
          ...
          {paginatedCustomerOrders.length > 0 ? (
            <TableBody sx={{ overflowY: "scroll" }}>
              {paginatedCustomerOrders.map((customerOrder, index) => {
                ...
                //     const updatedCustomerOrders = dataTables.filter(
                //       (order) => order.id !== id
                //     );
                //     setSelectedCustomerOrders((prevSelected) =>
                //       prevSelected.filter((selectedId) => selectedId !== id)
                //     );
                //     updateDataTable(updatedCustomerOrders);
                //   } catch (err) {
                //     console.log(err);
                //   }
                // };
                const handleShipOneCustomerOrders = async (
                  ids: string[]
                ): Promise<void> => {
                  try {
                    // Lặp qua danh sách các đơn hàng được chọn và gọi phương thức xóa
                    await KhoCoTyService.moveDataToCNDH(ids);
...
KhoCotyTable.propTypes = {
  dataTables: PropTypes.array.isRequired,
};

KhoCotyTable.defaultProps = {
  dataTables: [],
};

export default KhoCotyTable;
